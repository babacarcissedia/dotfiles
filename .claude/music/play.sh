#!/bin/bash
# Play a random 15s segment of a Gibran Alcocer piece when Claude is idle
...
# Pick a random start point (leave 15s room at the end)
...
# Extract 15s segment to temp file and play it
TEMP="/tmp/claude-piano-$$.wav"
ffmpeg -ss "$START" -t 15 -i "$RANDOM_FILE" -ar 44100 -ac 2 -f wav "$TEMP" -y -loglevel quiet 2>/dev/null
